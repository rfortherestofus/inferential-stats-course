...
```{r}
## 1 ##
# No significant difference in hs_gpa by grade_class, so no Tukey's HSD.
...
#Highest: English & math r = .83
#Lowest: English & reading r = .38
```
...
```{r}
college %>%
  lm(act_reading ~ iq + act_science, data = .) %>%
  lm.beta() %>%
  summary()
```
