...
## Alternative 2: this is not covered ##
college %>%
  t_test(act_english ~ gender, var.equal = FALSE,
...
1. Scores are **independent** from one another. .small[*Note*: this is not an assumption of within subjects designs like the dependent t-test or repeated measures ANOVA.]
...
- That have proportionate intervals between levels
...
Ordinal variables *do not have proportionate intervals between levels.*
...
college %>%
  select(id, iq) %>% 
  arrange(iq) %>%
  mutate(iq = DescTools::Winsorize(iq, #<<
                        minval = 84, #<<
                        maxval = 125)) #<<
```
.small[Winsorize() is from the DescTools package.]
]
...
??? 
Explain the 2.1 was because can't take square roots of zero or negative values, so needed to shift values up. 
