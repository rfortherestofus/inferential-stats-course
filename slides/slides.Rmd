...
```{r}
college %>%
  t.test(weight_2 ~ smokes, data = ., `var.equal = TRUE`)
```
...
```{r}
college %>%
  t.test(weight_2 ~ smokes, data = ., var.equal = TRUE, `alternative = "two.sided"`)
```
...
```{r}
college %>%
  t.test(weight_2 ~ smokes, data = ., var.equal = TRUE, `alternative = "greater"`)
```

```{r}
college %>%
  t.test(weight_2 ~ smokes, data = ., var.equal = TRUE, `alternative = "less"`)
```
...
college %>%
  filter(gender != "Trans*") %>%
  t.test(act_english ~ gender, data = ., var.equal = FALSE)
```
