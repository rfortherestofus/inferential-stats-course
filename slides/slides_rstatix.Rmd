...
1. Perform an independent samples t-test to test whether there is a difference in `act_english` by `gender`, but only for male or female students. Use `var.equal = TRUE`. Is there a difference? What is the p-value? 
...
college %>%
  ...
  pivot_longer(everything(), names_to = "time",
               values_to = "weight", names_prefix = "weight_") %>%
  t_test(weight ~ time, paired = TRUE) #<<
```
...
college %>%
  ...
  t_test(exam ~ time, paired = TRUE)

## 3 ##
college %>%
  select(exam_1, exam_2) %>%
  gather(key = "act_test", value = "score") %>%
...
```{r, eval = TRUE}
college %>% anova_test(exam_1 ~ grade_class)
```

--

```{r}
college %>% anova_test(exam_1 ~ grade_class, `detailed = TRUE`)
```
...
Notice that we'll need to make our data into *long format* to make this work, since we need the DV into into variable. Recall there are two ways to do this:
...
college %>%
  select(exam_1:exam_3, id) %>%
  gather(-id, key = "exam", value = "score") %>%
...
college %>%
  select(exam_1:exam_3, id) %>%
  gather(-id, key = "exam", value = "score") %>%
  anova_test(dv = score, wid = exam, within = exam) #<<
```
...
Two methods: the `rstatix` package or the `janitor` package
...
Prefer janitor because it's more tidyverse friendly than chisq_test is. There is a submitted issue on GitHub for the rstatix package to make this function pipe friendly, but until then janitor works great. 
...
Somtimes this works. In this case, it isn't sufficient. So let's go back. 
